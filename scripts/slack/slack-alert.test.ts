...
describe("Video Link Checker", () => {
  test("Returns video links, if REPORT_ARTEFACT_URL exists", async () => {
    ...
    const s = await slacker.getVideoLinks(REPORT_ARTEFACT_URL);
    expect(s).toContain(
      `<http://sometesturl.com/Users/you54f/dev/saftest/githubrepos/source_repos/cypress-slack-reporter/cypress/videos/test/cucumber-examples/BailOnStepFailure.feature.mp4|Video:- BailOnStepFailure.feature.mp4>`
    );
  });
...
describe("Screenshot Link Checker", () => {
  test("Returns undefined if screenshot dir is empty and if REPORT_ARTEFACT_URL exists", async () => {
    const REPORT_ARTEFACT_URL: string = "http://sometesturl.com";
    const s = await slacker.getScreenshotLinks(REPORT_ARTEFACT_URL);
    expect(s).toEqual(undefined);
  });
  test("Returns error, if REPORT_ARTEFACT_URL doesnt exist", async () => {
    const REPORT_ARTEFACT_URL: string = "";
...
describe("Get Files Checker", () => {
  test("Returns only specified filetypes if they exist in a dir", async () => {
    const dir: string = path.join(__dirname, "..", "..", "mochareports");
    const ext: string = ".html";
    const fileList: string[] = [];
    const s = await slacker.getFiles(dir, ext, fileList);
    expect(s).toEqual([
      "/Users/you54f/dev/saftest/githubrepos/source_repos/cypress-slack-reporter/mochareports/report-20190403-233435.html"
    ]);
  });
...
describe("Test Report Parser", () => {
  test("Reads a report a returns the results", async () => {
    ...
    const reportResult = {
      reportFile: [
        "/Users/you54f/dev/saftest/githubrepos/source_repos/cypress-slack-reporter/mochareports/report-20190403-233429.json"
      ],
      totalDuration: 94597,
      totalFailures: 0,
      totalPasses: 18,
  ...
  test("Returns an error when multiple json reports are found", async () => {
    ...
    expect(reportParser).toThrowError(new Error("Multiple json reports found, please run mochawesome-merge to provide a single report"));
  });
});
...
describe("Test Report URL Generator", () => {
  test("Reads a report a returns the results", async () => {
    ...
    expect(s).toEqual("report-20190403-233435.html");
  });
  ...
  test("Returns an error when multiple html reports are found", async () => {
    ...
    expect(reportParser).toThrowError(new Error("Multiple reports found, please provide only a single report"));
  });
});